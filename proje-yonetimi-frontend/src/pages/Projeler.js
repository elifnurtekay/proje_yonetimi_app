...
export default function Projeler() {
  ...
  const resolveCoordinates = async (payload) => {
    ...
    if (!query) {
      throw new Error("Adres alanları eksik. Lütfen en az şehir ve ilçe girin.");
    }
    if (!mapsApiKey) {
      throw new Error("Harita API anahtarı bulunamadı. .env dosyasını kontrol edin.");
    }
    ...
    if (rawLat == null || rawLng == null) {
      throw new Error("Adres koordinata çevrilemedi. Lütfen adresi daha detaylı girin.");
    }

    // 🔥 ÖNEMLİ KISIM: En fazla 6 ondalık basamak olacak şekilde stringe çeviriyoruz
    const latitude = Number(rawLat).toFixed(6);   // örn: "39.886579"
    const longitude = Number(rawLng).toFixed(6);  // örn: "32.712123"
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      const base = {
        ...
        // Kullanıcıdan alınmasın; adres ne ise ona göre tekrar geocode edilsin
        latitude: null,
        longitude: null,
  ...
  const handleAddSave = async (e) => {
    ...
    try {
      const base = {
        ...
        // Kullanıcıdan alınmıyor, hep otomatik hesaplanacak:
        latitude: null,
        longitude: null,
        geofence_radius: null, // istersen 100 gibi sabit de verebilirsin
      };

      const body = await resolveCoordinates(base); // burada geocodeAddress devreye giriyor
      const created = await addProject(body, token);
      const normalized = ensureEffectiveProgress(
        created,
