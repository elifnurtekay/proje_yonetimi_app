...
function buildProcessCard(process) {
  ...
  startEnd.innerHTML = `<span>Takvim</span><span>${process.start_date || "-"} → ${
    process.end_date || "-"
  }</span>`;
  ...
  coords.innerHTML = `<span>Koordinat</span><span>${
    lat && lng ? `${lat}, ${lng}` : "-"
  }</span>`;
  ...
  const progress = Math.max(
    0,
    Math.min(100, Number(process.effective_progress ?? process.progress ?? 0))
  );
...
export default function GoogleProcessMap({ processes = [] }) {
  ...
  const [mapReady, setMapReady] = useState(false); // 🔹 eklendi
  ...
  useEffect(() => {
    ...
    ensureGoogleMaps(apiKey)
      .then((google) => {
        ...
        mapRef.current = map;
        setMapReady(true); // 🔹 harita hazır bilgisi
      })
      .catch((err) => {
        setError(
          err.message || "Google Haritalar yüklenirken hata oluştu."
        );
      });
  }, [apiKey, validProcesses]);

  // Overlay kartları çiz
  useEffect(() => {
      ...
      if (overlay) {
        ...
        bounds.extend(
          new googleRef.current.maps.LatLng(
            Number(process.latitude),
            Number(process.longitude)
          )
        );
      }
    });
  ...
  }, [validProcesses, mapReady]); // 🔹 mapReady dependency eklendi
