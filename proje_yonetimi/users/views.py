...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        if not credential:
            return Response({"detail": "Google kimlik belirteci bulunamadı."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        except ValueError as exc:  # yanlış token
            return Response({"detail": "Google token doğrulanamadı.", "error": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
