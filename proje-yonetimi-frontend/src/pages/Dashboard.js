  ...
  useEffect(() => {
    ...
    fetchDashboardSummary(token)
      .then((data) => {
        ...
        // Backend -> UI mapping
        const mappedCards = {
          total_projects: Number(data?.toplam_proje ?? 0),
          active_tasks: Number(data?.aktif_gorev ?? 0),
          team_members: Number(data?.ekip_uyesi ?? 0),
          completed_tasks: Number(data?.tamamlanan ?? 0),
        };

        const mappedRecent = Array.isArray(data?.son_projeler) ? data.son_projeler : [];

        const mappedUpcoming = Array.isArray(data?.yaklasan_gorevler)
          ? data.yaklasan_gorevler.map((t) => ({
              id: t.id,
              title: t.title,
              status: t.status,
